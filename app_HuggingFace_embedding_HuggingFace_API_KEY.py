...
from langchain.llms import HuggingFaceHub  # ✅ Hugging Face Model for GPT Replacement
import os
import shutil
import faiss
import numpy as np
...
load_dotenv()
api_key = os.getenv("HUGGINGFACEHUB_API_TOKEN")  # ✅ Use Hugging Face API key
if not api_key:
    raise ValueError("API Key not found! Add it to your .env file.")

# ✅ Initialize Hugging Face Model for Text Generation (Replacing GPT-4)
llm = HuggingFaceHub(repo_id="tiiuae/falcon-7b-instruct", model_kwargs={"temperature": 0.7}, huggingfacehub_api_token=api_key)
...
# ✅ Example Query
query = "tell me hotel review in short words , good or bad"
...
if scores:
    ...
    if score_value >= score_threshold:
        ...
        print("\n\033[1;31m❌ Low Match (Similarity < 0.5), Hugging Face GPT will answer your query.\033[0m")
        use_faiss = False

#########################################################################################################################
# ✅ Define System Prompt to Improve Hugging Face GPT Accuracy
system_prompt = """
...
# ✅ Use Hugging Face GPT to Answer the Query Based on Retrieved Context
if not use_faiss:  # ✅ If FAISS is irrelevant, rely **only on Hugging Face GPT**
    prompt = f"Answer the following question:\n\nQuery: {query}\n\nAnswer:"
    gpt_response = llm.predict(prompt)
else:  
    gpt_response = retrieved_docs[0].page_content  # ✅ Direct FAISS answer, no GPT involved
...
if use_faiss:
    ...
    # ✅ FAISS is not relevant → Only Hugging Face GPT answer
    print("\n\033[1;36m🤖 **Hugging Face GPT-Generated Answer:**\033[0m\n")
    # print(f"\033[1m{gpt_response}\033[0m")  # Bold text output
